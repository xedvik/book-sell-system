...
@section('title', 'Редактирование продажи')
@section('header', 'Редактирование продажи')
@section('page-title', 'Редактирование продажи #' . $sell->id)
...
@section('breadcrumbs')
    <nav class="text-sm text-gray-500 mb-4">
        <ol class="list-none p-0 inline-flex">
            ...
            <li>
                <span>Редактирование #{{ $sell->id }}</span>
            </li>
        </ol>
    </nav>
@endsection
...
@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <form action="{{ route('a-panel.sales.update', $sell->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <h3 class="text-lg font-medium text-gray-900">Информация о продаже</h3>
                    <p class="mt-1 text-sm text-gray-500">Обновите информацию о продаже.</p>
                </div>
                ...
                <div class="grid grid-cols-6 gap-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="book_id" name="book_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            ...
                            @foreach($books ?? [] as $book)
                                <option value="{{ $book->id }}" {{ (old('book_id', $sell->book_id) == $book->id) ? 'selected' : '' }}>
                    ...
                    <div class="col-span-6 sm:col-span-2">
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $sell->price) }}" step="0.01" min="0" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        @error('price')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
